...
%   dV - delta-V required for propulsion [km/s]
    ...
    [tof,dV,beta] = spiraling(method,flybys.planet,desired_radius);
...
%     end
    ...
    t_observation = observation_time(desired_radius, orbit.inclination); % >38 days per orbit
end
...
function [tof,dV,beta] = spiraling(method,flyby_planet,desired_radius)
% assumptions:
...
% for solar sail use same values as POLARIS
    ...
    if method == "Solar Sail"
     ...
     if method == "Ion"
       dV = sqrt(mu/desired_radius) - sqrt(mu/initial_radius^1);
       period = 2*pi*sqrt( ((desired_radius+initial_radius)/2) ^3/mu);
       tof = 120 * period / 86400; % SMAD table 9-16 says 60-120 periods; if m = 2000kg, 
                                   % a  = 0.0001g and tof = 120*period
     end
end
...
function t_observation = observation_time(desired_radius, desired_inclination)
    ...
    t_observation = t_observation / 86400;
end
